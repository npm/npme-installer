  ...
  lab.experiment('getos', function() {
    lab.it('returns default platform if getos returns an error', function(done) {
      (new Platform({
        ...
        Lab.expect(platform.toString()).to.match(/Apt/) // story checks out, we're running apt.
        done();
      });
    });
    ...
    lab.it('returns centos installer if codename = latest and release = 6.5', function(done) {
      (new Platform({
        ...
        Lab.expect(platform.toString()).to.match(/Yum/) // story checks out, we're running apt.
        done();
      });
    });
